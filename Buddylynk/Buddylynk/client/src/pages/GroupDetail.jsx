                        ...
                        return (
                            ...
                            >
                                <div className="relative">
                                    <div className={`${post.media ? 'w-full max-w-full' : 'max-w-[85vw] sm:max-w-[70vw] md:max-w-[60vw] lg:max-w-[50vw]'} px-1 py-1 rounded-2xl ${isOwn
                                        ...
                                        {post.media && (
                                            <div className="mb-2 rounded-xl">
                                                {Array.isArray(post.media) && post.media.length > 0 ? (
                                                    // New array format - handles both single and multiple media
                                                    post.media.length === 1 ? (
                                                        // Single media item in array
                                                        <SensitiveMediaWrapper isSensitive={post.media[0]?.isNsfw || post.isNsfw}>
                                                            <div className="w-full">
                                                                {post.media[0]?.type === 'video' || post.media[0]?.url?.match(/\.(mp4|webm|mov)$/i) ? (
                                                                    <div className="w-full" style={{ willChange: 'transform', transform: 'translateZ(0)' }}>
                                                                        <VideoPlayer src={post.media[0].url} className="w-full" />
                                                                    </div>
                                                                ) : post.media[0]?.type === 'image' || post.media[0]?.url?.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                                                                    <div className="w-full magic-frame-container" style={{ willChange: 'transform', transform: 'translateZ(0)' }}>
                                                                        <SafeImage
                                                                            src={post.media[0].url}
                                                                            alt="Media"
                                                                            onClick={() => { setFullScreenImages(post.media.filter(m => m.type === 'image' || m.url?.match(/\.(jpg|jpeg|png|gif|webp)$/i)).map(m => ({ url: m.url, type: 'image' }))); setFullScreenIndex(0); }}
                                                        ...
                                                        // Multiple media items - vertical stack layout
                                                        <div className="w-full magic-frame-container" style={{ willChange: 'transform', transform: 'translateZ(0)' }}>
                                                            <div className="flex flex-col gap-1 bg-black rounded-lg overflow-hidden">
                                                                {post.media.map((mediaItem, idx) => (
                                                                    <SensitiveMediaWrapper key={idx} isSensitive={mediaItem?.isNsfw || post.isNsfw}>
                                                                        <div className="w-full" style={{ willChange: 'transform', transform: 'translateZ(0)' }}>
                                                                            {mediaItem?.type === 'video' || mediaItem?.url?.match(/\.(mp4|webm|mov)$/i) ? (
                                                    ...
                                                    // Old format - single media URL string (backward compatibility)
                    ...
                    >
                                ...
                                >
                                    {mediaPreviews.length === 1 ? (
                                        ...
                                        <div className="rounded-xl overflow-hidden">
                                            ...
                                            {/* Small images row */}
                                            <div className={`grid gap-0.5 ${mediaPreviews.length === 3 ? 'grid-cols-2' :
                        ...
                        <div className="px-4 py-4 bg-black/80 flex gap-3">
                            <button
                                ...
                                disabled={isSending}
                                className={`flex-1 py-3 rounded-xl font-medium flex items-center justify-center gap-2 ${isSending
