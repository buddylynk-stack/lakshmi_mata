...
const addPostToGroup = async (req, res) => {
    try {
        ...
        console.log(`📝 Adding post to group ${id}`);
        ...
        if (req.files && req.files.length > 0) {
            console.log(`📤 Uploading ${req.files.length} file(s) to S3...`);

            for (let i = 0; i < req.files.length; i++) {
                const file = req.files[i];
                console.log(`   [${i + 1}/${req.files.length}] ${file.originalname} (${file.mimetype}, ${(file.size / 1024).toFixed(1)}KB)`);
                ...
                console.log(`   ✅ Uploaded: ${mediaType} -> ${mediaUrl.substring(0, 60)}...`);
            }

            console.log(`📦 Total media array: ${mediaArray.length} item(s)`);
        }
        ...
        console.log(`✅ Post added to group ${id} successfully with ${mediaArray.length} media item(s)`);
